# nunu-build-here.ps1 — build and drop straight into your folder (no SDK hunting)

# === HARD PATHS ===
$RepoRoot      = 'C:\NunuCompanionApp V2.0'
$DeployDir     = 'C:\NunuCompanionApp V2.0\Nunu Companion App V2.0\Dployed NunuCompanionAppV2.0'
$DalamudDir    = 'C:\Users\insan\AppData\Roaming\XIVLauncher\addon\Hooks\dev'   # where Dalamud.dll lives
...
# (optional) embed Persona\DefaultPersona.json if it exists
$personaPath = Join-Path $ProjDir 'Persona\DefaultPersona.json'
if(Test-Path -LiteralPath $personaPath){
  $emb = $ig2.SelectSingleNode("EmbeddedResource[@Include='Persona\DefaultPersona.json']")
  if(-not $emb){ $emb = $xml.CreateElement('EmbeddedResource'); $emb.SetAttribute('Include','Persona\DefaultPersona.json'); [void]$ig2.AppendChild($emb) }
}
...
# 5) Deploy straight to your folder
